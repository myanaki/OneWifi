                                ...
                                bus_user_data_t *user_data)
{
    ...
    wifi_util_dbg_print(WIFI_CTRL, " Sachin_debug: In %s:%d \n", __func__, __LINE__);
    if (mgr == NULL || ctrl == NULL) {
        wifi_util_error_print(WIFI_CTRL, "%s:%d NULL pointers\n", __func__, __LINE__);
        return bus_error_general;
    }
    ...
    wifi_util_dbg_print(WIFI_CTRL, " Sachin_debug: Exit %s:%d \n", __func__, __LINE__);
    webconfig_data_free(data);
    free(data);
    return bus_error_success;
}
...
bus_error_t set_force_vap_apply(char *name, raw_data_t *p_data, bus_user_data_t *user_data)
{
    ...
    wifi_util_error_print(WIFI_CTRL, "Sachin_debug In: %s:%d \n", __func__,
            __LINE__);
    ...
    if (ret == 1 && idx > 0 && idx <= num_of_radios * MAX_NUM_VAP_PER_RADIO) {
        ...
        webconfig_data_free(data);
        free(data);
        wifi_util_error_print(WIFI_CTRL, "Sachin_debug Exit Success: %s:%d \n", __func__,
            __LINE__);
        return bus_error_success;
    }
    wifi_util_error_print(WIFI_CTRL, "Sachin_debug Exit: %s:%d \n", __func__,
            __LINE__);
