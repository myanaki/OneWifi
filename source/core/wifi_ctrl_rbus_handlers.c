...
bus_error_t set_endpoint_enable(char *name, raw_data_t *p_data, bus_user_data_t *user_data)
{
    ...
    if (rf_status) {
       ...
       wifi_util_info_print(WIFI_CTRL, "IGNITE_RF_DOWN: Docsis enabled. Stoping Station Vaps and multiap_rfc: %d\n", rfc_param->multiap_rfc);
       if (rfc_param->multiap_rfc) {
            wifi_util_info_print(WIFI_CTRL, "IGNITE_RF_DOWN: MultiAP RFC enabled. Starting MultiAP Vaps\n");
...
int set_to_extender_mode(bus_handle_t *handle, const char *paramNames, unsigned int data_value, int type)
{
    ...
    wifi_util_info_print(WIFI_MGR, "%s:%d bus: wifi bus set[%s]:value:%d\r\n", __func__, __LINE__,
