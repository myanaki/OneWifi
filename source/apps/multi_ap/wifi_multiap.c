    ...
    void *out_buff, size_t out_len)
{
    ...
    while (remaining > (int)sizeof(multiap_tlv_t) && tlv->type != multiap_tlv_type_eom) {
        ...
        if (tlv_len > remaining - (int)sizeof(multiap_tlv_t)) {
            wifi_util_info_print(WIFI_CTRL,
                "TLV length exceeds remaining buffer, aborting. ******* \n");
            return -1;
        }
    ...
    case multiap_tlv_type_supported_service: {
        ...
        wifi_util_info_print(WIFI_CTRL,
            "%s:%d stlv->supported_service = 0x%x  htons value =0x%x normalval=%d \n", __func__,
            __LINE__, stlv->supported_service, htons(tlv->len), tlv->len);
...
int handle_autoconf_search(unsigned char *data, unsigned int len)
{
    ...
    // wifi_util_error_print(WIFI_CTRL,"supported_service = %d:
    // %s:%d\n",supported_service,__func__,__LINE__); if(device_supporting_service ==
    // multiap_service_type_extender || supported_service == multiap_service_type_extender)
    ...
    if (device_supporting_service == multiap_service_type_extender ||
        ...
        wifi_util_error_print(WIFI_CTRL,
            "either supporting service or supported service is extender so not replying\n");
        return -1;
    }
...
int handle_autoconf_search_resp(unsigned char *data, unsigned int len)
{
    ...
    // uint8_mac_to_string_mac(dst,st);
    // wifi_util_error_print(WIFI_CTRL,"Enter %s:%d received sta_mac_addr
    // offset=%d\n",__func__,__LINE__,offset); This is the retriving mechanism from the TLV int
    // total_macs = offset / MAC_ADDR_LEN;
...
int create_autoconfig_resp_msg(unsigned char *buff, unsigned char *dst, char *interface_name)
{
    ...
    for (itr = 0; itr < getNumberRadios(); itr++) {
        wifi_util_info_print(WIFI_CTRL, "%s:%d index=%d and  getNumberRadios()=%d", __func__,
